...
type TranslatableWord = { word: string; translation: string };
...
type ActiveBubble =
  | { type: 'word'; questionId: number; wordIndex: number }
  | { type: 'sentence'; questionId: number }
  | null;
...
export default function FillInTheBlanks({
  ...
  const passedCurrentAttempt = hasChecked && correctAnswers >= 7;
  ...
  useEffect(() => {
    function closeBubble(event: MouseEvent) {
      ...
      if (
        ...
        !target.closest(`.${styles.translationAnchor}`)
      ) {
        setActiveBubble(null);
      }
    }

    document.addEventListener('mousedown', closeBubble);

    return () => document.removeEventListener('mousedown', closeBubble);
  }, []);
  ...
  async function handleCheckAnswers() {
    ...
    const { data, error } = await supabase.rpc('save_lesson_attempt', {
    ...
    if (error || !data) {
      setProgressError(
        error?.message ?? 'No se pudo guardar tu resultado. Inténtalo de nuevo.',
      );
  ...
  async function setCompletion(completed: boolean) {
    ...
    const { data, error } = await supabase.rpc('set_lesson_completion', {
      p_lesson_key: lessonKey,
  ...
  async function playAudio(text: string) {
    ...
    try {
      ...
      if (!audioBlob.size || requestId !== audioRequestIdRef.current) {
        return;
      }
        ...
        <button
          ...
          onClick={() =>
            setActiveBubble(
              isOpen
                ? null
                : { type: 'word', questionId, wordIndex },
            )
          }
        >
          {item.word}
